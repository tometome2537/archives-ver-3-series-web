...
export default function EntitySelector({ setEntityId }: EntitySelectorProps) {
  ...
  const [apiDataBelongHistery, setApiDataBelongHistery] = useState<Array<BelongHisteryObj>>([]);
  // Entity Organisation Idと Entity Person IDで、そのパーソンが組織に所属しているかどうかを返す
  const isPersonInOrganization = (entityPersonId: string, entityOrganizationId: string): BelongHisteryObj | undefined => {
    return apiDataBelongHistery.find((item: BelongHisteryObj) => {
      // 条件に一致するアイテムがある場合 true を返す
      return item["entityId"] === entityPersonId && item["entityOrganizationId"] === entityOrganizationId;
    });
  }
  ...
  const [errorBelongHistery, setErrorBelongHistery] = useState<string | null>(null);
  ...
  return (
    <div>
      ...
      <Button variant="outlined" onClick={handleClickOpenOrganisation}>
        {selectEntityIdOrganisation.length !== 0 ? selectEntityIdOrganisation[0]["name"] : "グループを選択"}
      </Button>
      <Button variant="outlined" onClick={handleClickOpenPerson}>
        {selectEntityIdPerson.length !== 0 ? selectEntityIdPerson[0]["name"] : "アーティストを選択"}
      </Button>
              ...
              ? apiDataEntity.map((item: EntityObj, index: number) => (
                <>
                  ...
                  {item["category"] === "organization" && (
                    <option
                      key={index}
                      selected={selectEntityIdOrganisation.length > 0 ? item["id"] === selectEntityIdOrganisation[0]["id"] : false}
                      value={item["id"]}
                    >
                      {item["name"]}
                    </option>
              ...
              ? apiDataEntity.map((item: EntityObj, index: number) => (
                <>
                  ...
                  {item["category"] === "person" &&
                    ...
                    isPersonInOrganization(item["id"], selectEntityIdOrganisation[0]["id"]) &&
                    <option
                      key={index}
                      selected={selectEntityIdPerson.length > 0 ? item["id"] === selectEntityIdPerson[0]["id"] : false}
                      value={item["id"]}
                    >
                      {item["name"]}
                    </option>
