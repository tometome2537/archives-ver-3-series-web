...
export default function SearchBar(props: SearchBarProps) {
	...
	// 日付ダイアログで送信が押された場合
	const handleDialogDateSubmit = (event: React.FormEvent<HTMLFormElement>) => {
	...
	function filter(
		...
		const hiragana = toHiragana(inputValueLowerCase).replace(/[a-zA-Z]/g, "");

		// optionsリストをフィルタリングし、inputValueがオプションのラベルに含まれるものを返す
		return options.filter((option) => {
			// 入力途中の文字数に応じてフィルタリングを行う。
			if (option.queryMinLengthForSuggestions) {
				if (inputValueLowerCase.length < option.queryMinLengthForSuggestions) {
					return false;
				}
			}
	...
	const getSearchSuggestionFromLabel = (
		...
		const r = props.searchSuggestions?.find(
			(item) => item.label === label && item.categoryLabel === categoryLabel,
		);
		if (r) {
			return r;
		}
			...
			<Autocomplete
							...
							{React.Children.map(
								...
								(child) => (
									<Box
										...
										sx={{
											// タグの横幅を定義
											width: props.showTagCount ? "100%" : "32%",
										}}
									>
										{/* 各要素にユニークなkeyを設定 */}
										{
											...
											"children" in child.props ? (
												<>
													...
													{React.cloneElement(<Chip />, {
														...
														...(() => {
															const suggestion = props.showTagIcon
															...
															return {
																...
																avatar: suggestion?.imgSrc ? (
																	<Avatar
																		alt={String(child.props.children)}
																		src={suggestion.imgSrc}
																	/>
																) : suggestion?.icon ? undefined : (
																	<Avatar>
																		{child.props.children
																			? String(child.props.children)[0]
																			: ""}
																	</Avatar>
																),
															};
				...
				renderValue={(value, getItemProps) =>
					value.map((option, index) => {
						...
						const isFixed =
							props.fixedOptionValues?.includes(option.value) ?? false;
						...
						return (
							...
							>
								<Chip
									...
									icon={props.showTagIcon === false ? undefined : option.icon}
									avatar={
										props.showTagIcon === false ? undefined : option.imgSrc ? (
											<Avatar alt={option.label} src={option.imgSrc} />
								...
								{props.availableCategoryIds &&
									!props.availableCategoryIds.includes(option.categoryId) && (
										<CloseIcon
											sx={{
												position: "absolute",
												top: "10%",
					...
					<DialogContent>
						...
						<Stack>
							<TextField
								...
								margin="dense"
								id="label"
								defaultValue={formatDate(dialogDatePickerValue.value)}
								onChange={(event) =>
									setDialogDatePickerValue({
						...
						<Stack>
							<TextField
								...
								margin="dense"
								id="value"
								defaultValue={convertStringToDate(dialogDatePickerValue.value)}
								onChange={(event) =>
									setDialogDatePickerValue({
										...dialogDatePickerValue,
						...
						<Stack direction="row">
							...
							<TextField
								...
								margin="dense"
								id="categoryLabel"
								defaultValue={dialogDatePickerValue.categoryLabel}
								onChange={(event) =>
									setDialogDatePickerValue({
										...dialogDatePickerValue,
