...
import type { MediaItem, useAppleMusic } from "@/contexts/AppleMusicContext";
...
export function usePlayerSwitching(
	...
	const findPlaylistItem = useCallback(
		(mediaId?: string) => {
			...
			return playerPlaylist.videos.find((item) => item.mediaId === mediaId);
		},
	...
	useEffect(() => {
		...
		if (repeat && youTubePlayer) {
			...
			const playListIndex = playerPlaylist.videos.findIndex(
				(item) => item.mediaId === youTubePlayerState.getVideoData.video_id,
			);
...
export function usePlayerLayout(
	...
	const aspectRatio = useMemo(() => {
		...
		if (playerItem?.arWidth && playerItem.arHeight) {
	...
	const width = useMemo(() => {
		if (isFullscreen) {
			if (isMobile) {
				return youTubePlayerState?.getVideoData.author.endsWith(" - Topic")
					? screenWidth * 0.8
					: screenWidth;
			}
